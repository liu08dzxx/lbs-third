...
import com.vtradex.ehub.third.lbsthird.jtt809.Jtt809Util;
import com.vtradex.ehub.third.lbsthird.util.ByteBufPool;
import com.vtradex.ehub.third.lbsthird.util.CRC16CCITT;
...
public class Message implements Serializable {
	...
	public Message(ByteBuf buffer) {
		...
		// }
//		LOGGER.debug("可读长度：{}",buffer.readableBytes());
        //如果经过了加密，则解密
        if(encryptFlag==1) {
...
//		/** 开始发送 */
//		// 写头
//		ByteBuf bb = ByteBufPool.BYTE_BUF_POOL.buffer(1);
//		bb.writeByte(Message.MSG_HEAD);
//		ctx.write(bb);
//		// 写中间
//		ByteBuf byteBuf = ByteBufPool.BYTE_BUF_POOL.buffer((int) msgLength);
//		buildHeadAndBody(byteBuf);
//		// 转义,不知道转义后的具体长度
//				ByteBuf formatBuffer = ByteBufPool.BYTE_BUF_POOL.buffer();
//				Jtt809Util.formatBuffer(byteBuf.array(), formatBuffer);
//		ctx.write(formatBuffer);
//		// 写尾
//		bb = ByteBufPool.BYTE_BUF_POOL.buffer(1);
...
//		ctx.writeAndFlush(bb);	
		...
		ctx.write(bb);
		// 写中间
		ByteBuf bbs=buildHeadAndBody(this);
		ctx.write(bbs);
		// 写尾
	...
	public void send(Channel channel) {
		...
		buildHeadAndBody(byteBuf);
		// 转义,不知道转义后的具体长度
		ByteBuf formatBuffer = ByteBufPool.BYTE_BUF_POOL.buffer((int)msgLength*2);
		Jtt809Util.formatBuffer(byteBuf.array(), formatBuffer);
		byteBuf.release();	
	 ...
	 * @return
	 */
	private  void buildHeadAndBody(ByteBuf byteBuf ) {

		byteBuf.writeInt((int)msgLength);// 4byte 数据长度
		byteBuf.writeInt(msgSn);// 4byte 报文序列号
		byteBuf.writeShort(msgId);// 2byte 业务数据类型
		byteBuf.writeInt(msgGesscenterId);// 4byte 下级平台接入码
		byteBuf.writeBytes(versionFlag);// 3byte版本
		byteBuf.writeByte((int) encryptFlag);// 1byte
		byteBuf.writeInt((int) encryptKey);// 4byte
		if(msgBody!=null) {
			// 判断是否需要加密
			if (encryptFlag == 1) {
//				ByteBuf bytebody = msgBody;
				byte[] body = msgBody.array();
				body = Jtt809Util.encrypt(5, 8, 10, (int) encryptKey, body);
				byteBuf.writeBytes(body);
			} else {
